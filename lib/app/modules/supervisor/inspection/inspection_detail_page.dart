...
import 'inspection_detail_controller.dart';
...
class InspectionDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final controller = Get.put(InspectionDetailController());
    ...
    return Scaffold(
      ...
      body: Obx(() {
        ...
        if (controller.isLoading.value &&
            controller.inspectionDetail.value == null) {
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                ElevatedButton.icon(
                  onPressed: controller.loadInspectionDetails,
        ...
        final inspection = controller.inspectionDetail.value;
        if (inspection == null) {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (inspection.remarks != null &&
                        inspection.remarks!.isNotEmpty) ...[
                      const _SectionTitle(title: 'Supervisor Remarks'),
                      const SizedBox(height: 10),
                      _SupervisorRemarksCard(remarks: inspection.remarks!),
                      const SizedBox(height: 16),
                    ],
                    ...
                    if (inspection.responses.isNotEmpty) ...[
                      _SectionTitle(
                        title:
                            'Inspection Items (${inspection.responses.length})',
                      ),
                      const SizedBox(height: 10),
                      ...inspection.responses.map(
                        (response) => _InspectionItemCard(response: response),
                      ),
                    ],

                    // Comments Section
                    if (inspection.companyName.isNotEmpty) ...[
                      const SizedBox(height: 16),
                      const _SectionTitle(title: 'Company'),
                      const SizedBox(height: 10),
                      _CommentsCard(comments: inspection.companyName),
                    ],
        ...
        child: Column(
          ...
          children: [
            ...
            _InfoRow(
              ...
              label: 'Submitted',
              value: _formatDateTime(inspection.createdAt),
            ),
          ],
        ),
      ),
...
class _InspectionItemCard extends StatelessWidget {
  final response;
  const _InspectionItemCard({required this.response});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Photo
            if (response.photo != null && response.photo!.isNotEmpty) ...[
              const SizedBox(height: 12),
              ClipRRect(
                borderRadius: BorderRadius.circular(8),
                child: Image.network(
